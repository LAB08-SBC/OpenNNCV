...
% scale eh a escala aplicada para se obter uma imagem 8x6
scale = 1/20;
...
ANNmarksI = zeros((width*scale)*(heigth*scale)*3,totalDataSet*(int32(1/(scale*scale))));
...
folder='C:\Users\Gilmar Jeronimo\Desktop\Testes MATLAB\DataBase\FieldMarks\';
folder='Users/junior/Desktop/Testes MATLAB/DataBase/FieldMarks/';
type = 'M_';
...
for i = beginDB:lastDB
    ...
    % Verifica se eh colorida ou preto e branco
    ...
    for frame = 1:(int32(1/(scale*scale)))
        ...
        % Salva o vetor 1D em ANN marks Inputs
        ANNmarksI(:,index) = segImage(:,img,frame);
...
save('ANNmarksI.mat','ANNmarksI');
save('ANNmarksL.mat','ANNmarksL');
...
for hiddenNeurons = 1:12
    ...
    netFilter = train(netFilter,ANNmarksI(:,1:(trainDataSet*int32(1/(scale*scale)))),ANNmarksL(1:(trainDataSet)*(int32(1/(scale*scale)))));
...
%% VALIDANDO FILTRO COM DADOS
    ...
    for i = 1:totalDataSet
        ...
        for j = 1:(int32(1/(scale*scale)))
            if(round(netFilter(ANNmarksI(:,index))) == ANNmarksL(index))
