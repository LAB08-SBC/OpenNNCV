...
%ANNgoalI = zeros((width*scale)*(heigth*scale)*3,totalDataSet*(int32(1/(scale*scale))));
ANNgoalI = zeros((width*scale)*(heigth*scale),totalDataSet*(int32(1/(scale*scale))));
...
folder='C:\Users\Gilmar Jeronimo\Desktop\Testes MATLAB\DataBase\Goal\';
%folder='Users/junior/Desktop/Testes MATLAB/DataBase/Goal/';
type = 'BCGL_G';
...
for i = beginDB:lastDB
    ...
    for frame = 1:(int32(1/(scale*scale)))
        ...
        ANNgoalI(:,index) = segImage(:,img,frame);
...
save('ANNgoalI.mat','ANNgoalI');
save('ANNgoalL.mat','ANNgoalL');
...
for hiddenNeurons = 1:12
    ...
    netFilter = train(netFilter,ANNgoalI(:,1:(trainDataSet*int32(1/(scale*scale)))),ANNgoalL(1:(trainDataSet)*(int32(1/(scale*scale)))));
...
%     img = 55; 
%     akk2 = uint8(reshape(ANNgoalI(:,(((img-1)*(int32(1/(scale*scale))))+1):(img*int32(1/(scale*scale)))),width*scale,heigth*scale,3,(int32(1/(scale*scale)))));
...
%% VALIDANDO FILTRO COM DADOS
    ...
    for i = 1:totalDataSet
        ...
        for j = 1:(int32(1/(scale*scale)))
            if(round(sim(netFilter,ANNgoalI(:,index))) == ANNgoalL(index))
...
save(string('ANNvalidationsGoal')+type+string('.mat'),'ANNvalidations');
