...
for i = inicio:fim 
    ...
    ANNimagens(:,:,j) = imresize(fftshift(log(1+abs(F))),reduz); %log(1+abs(F))
...
for hiddenNeurons = 1:35
    ...
    netR=train(netR,ANNimagens,ANNlabels,'reduction',200);
...
%     save('WR.mat','WR');
    ...
    for i=(fim+1):321
        A = double(imresize(fftshift(log(1+abs(fft2(imread(strcat(pasta,tipo,int2str(i),'.jpg')))))),reduz));
