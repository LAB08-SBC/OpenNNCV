...
for r = 1:35
    ...
    netPos=train(netPos,double(ANNimagensS(:,1:trainPic)),ANNlabels(1:2,1:trainPic));
    netSize=train(netSize,double(ANNimagensS(:,1:trainPic)),ANNlabels(3,1:trainPic));

    %% VALIDA?A?O
    for j = 1:total
        ANNresult(j,:) = [((j-1)+inicio) sim(netPos,double(ANNimagensS(:,j)))' sim(netSize,double(ANNimagensS(:,j)))'];
    end
