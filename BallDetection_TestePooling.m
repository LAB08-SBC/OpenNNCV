...
pasta = 'C:\Users\Gilmar Correia\Documents\Documentos\Projetos\GitHub\RNA-OpenCV\Lições\LaplacianFilter\';
tipo = 'BCG';
...
for i = inicio:321
    ...
    ANNimagensS(:,(i-inicio+1)) = reshape(E,(width/8)*(heigth/8),1);
...
for r = 1:35
    netPos=feedforwardnet(r);
    netPos.trainParam.max_fail = 100;
    netPos=train(netPos,double(ANNimagensS(:,1:trainPic)),ANNlabels(:,1:trainPic));

    %% VALIDAÇÃO
    for j = 1:total
        ANNresult(j,:) = [((j-1)+inicio) sim(netPos,double(ANNimagensS(:,j)))'];
    end
